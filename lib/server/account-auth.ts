...
import {
  isAccountSqliteConfigured,
  sqliteCreateSession,
  sqliteCreateUser,
  sqliteDeleteSession,
  sqliteFindUserById,
  sqliteFindUserByUsername,
  sqliteGetCurrentAccount,
  sqliteMarkActivationCodeUsed,
  sqliteRegisterWithCode,
  sqliteTouchLogin,
  sqliteUpdatePassword,
  sqliteValidateActivationCode,
} from "./account-sqlite";
...
export function isAccountStorageConfigured(): boolean { return isAccountSqliteConfigured() || Boolean(process.env.SUPABASE_URL && process.env.SUPABASE_SERVICE_ROLE_KEY); }
export async function updateUserPassword(userId: string, password: string): Promise<void> { if (isAccountSqliteConfigured()) { sqliteUpdatePassword(userId, hashPassword(password), new Date().toISOString()); return; } const result = await supabaseRestFetch<unknown[]>(`app_users?id=eq.${encodeSupabaseFilter(userId)}&select=id`, { method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify({ password_hash: hashPassword(password), updated_at: new Date().toISOString() }) }); if (!result.ok) throw new Error(result.error); }
export async function findUserByUsername(username: string): Promise<AppUserRecord | null> { if (isAccountSqliteConfigured()) return sqliteFindUserByUsername(username); const result = await supabaseRestFetch<AppUserRecord[]>(`app_users?username=eq.${encodeSupabaseFilter(username)}&select=id,username,password_hash,display_name,status,created_at,updated_at&limit=1`); if (!result.ok) throw new Error(result.error); return result.data[0] ?? null; }
export async function findUserById(id: string): Promise<AppUserRecord | null> { if (isAccountSqliteConfigured()) return sqliteFindUserById(id); const result = await supabaseRestFetch<AppUserRecord[]>(`app_users?id=eq.${encodeSupabaseFilter(id)}&select=id,username,password_hash,display_name,status,created_at,updated_at&limit=1`); if (!result.ok) throw new Error(result.error); return result.data[0] ?? null; }
...
export async function validateActivationCode(code: string): Promise<ActivationCodeRecord> { if (isAccountSqliteConfigured()) return sqliteValidateActivationCode(code); const result = await supabaseRestFetch<ActivationCodeRecord[]>(`activation_codes?code=eq.${encodeSupabaseFilter(code)}&select=code,status,max_uses,used_count,expires_at&limit=1`); if (!result.ok) throw new Error(result.error); const record = result.data[0]; if (!record) throw new Error("激活码不存在。"); if (record.status !== "active") throw new Error("激活码不可用。"); const maxUses = Number(record.max_uses); const usedCount = Number(record.used_count); if (!Number.isFinite(maxUses) || !Number.isFinite(usedCount) || usedCount >= maxUses) throw new Error("激活码已被使用完。"); const expiresAt = cleanAccountText(record.expires_at, 80); if (expiresAt && Date.parse(expiresAt) <= Date.now()) throw new Error("激活码已过期。"); return record; }
export async function markActivationCodeUsed(code: string, userId: string): Promise<void> { if (isAccountSqliteConfigured()) { sqliteMarkActivationCodeUsed(code, userId, new Date().toISOString()); return; } const current = await validateActivationCode(code); const usedCount = Number(current.used_count); const result = await supabaseRestFetch<unknown[]>(`activation_codes?code=eq.${encodeSupabaseFilter(code)}&select=code`, { method: "PATCH", headers: { Prefer: "return=representation" }, body: JSON.stringify({ used_count: usedCount + 1, last_used_by: userId, last_used_at: new Date().toISOString(), updated_at: new Date().toISOString() }) }); if (!result.ok) throw new Error(result.error); }
export async function createUser(input: { username: string; password: string; displayName?: string }): Promise<AppUserRecord> { const now = new Date().toISOString(); const payload = { id: createAccountId(), username: input.username, password_hash: hashPassword(input.password), display_name: cleanAccountText(input.displayName, 80) || input.username, status: "active", activated_at: now, last_login_at: now, created_at: now, updated_at: now }; if (isAccountSqliteConfigured()) return sqliteCreateUser(payload); const result = await supabaseRestFetch<AppUserRecord[]>("app_users?select=id,username,password_hash,display_name,status,created_at,updated_at", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify(payload) }); if (!result.ok) throw new Error(result.error); const user = result.data[0]; if (!user) throw new Error("账号创建失败。"); return user; }
...
export async function registerAccountWithCode(input: { username: string; password: string; displayName?: string; activationCode: string }): Promise<AppUserRecord> { if (isAccountSqliteConfigured()) { const now = new Date().toISOString(); try { return sqliteRegisterWithCode({ id: createAccountId(), username: input.username, password_hash: hashPassword(input.password), display_name: cleanAccountText(input.displayName, 80) || input.username, status: "active", activated_at: now, last_login_at: now, created_at: now, updated_at: now, activationCode: input.activationCode }); } catch (error) { throw new Error(mapRegisterError(String(error))); } } const result = await supabaseRestFetch<AppUserRecord | AppUserRecord[]>("rpc/app_register_account", { method: "POST", headers: { Prefer: "return=representation" }, body: JSON.stringify({ p_id: createAccountId(), p_username: input.username, p_password_hash: hashPassword(input.password), p_display_name: cleanAccountText(input.displayName, 80) || input.username, p_code: input.activationCode }) }); if (!result.ok) throw new Error(mapRegisterError(result.error)); const user = (Array.isArray(result.data) ? result.data[0] : result.data) as AppUserRecord | undefined; if (!user || !cleanAccountText(user.id, 120)) throw new Error("账号创建失败。"); return user; }
export async function touchUserLogin(userId: string): Promise<void> { const now = new Date().toISOString(); if (isAccountSqliteConfigured()) { sqliteTouchLogin(userId, now); return; } await supabaseRestFetch<unknown[]>(`app_users?id=eq.${encodeSupabaseFilter(userId)}&select=id`, { method: "PATCH", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ last_login_at: now, updated_at: now }) }); }
export async function createSession(userId: string, request: Request): Promise<{ token: string; expiresAt: string }> { const token = createSessionToken(); const now = new Date().toISOString(); const expiresAt = new Date(Date.now() + ACCOUNT_SESSION_MAX_AGE_SECONDS * 1000).toISOString(); if (isAccountSqliteConfigured()) { sqliteCreateSession({ tokenHash: hashSessionToken(token), userId, userAgent: cleanAccountText(request.headers.get("user-agent"), 500), expiresAt, now }); return { token, expiresAt }; } const result = await supabaseRestFetch<unknown[]>("app_sessions", { method: "POST", headers: { Prefer: "return=minimal" }, body: JSON.stringify({ token_hash: hashSessionToken(token), user_id: userId, user_agent: cleanAccountText(request.headers.get("user-agent"), 500), expires_at: expiresAt, created_at: now, last_seen_at: now }) }); if (!result.ok) throw new Error(result.error); return { token, expiresAt }; }
export async function deleteSessionToken(token: string): Promise<void> { if (!token) return; const tokenHash = hashSessionToken(token); if (isAccountSqliteConfigured()) { sqliteDeleteSession(tokenHash); return; } await supabaseRestFetch<unknown[]>(`app_sessions?token_hash=eq.${encodeSupabaseFilter(tokenHash)}`, { method: "DELETE" }); }
...
export async function getCurrentAccount(request: Request): Promise<AppAccount | null> {
  ...
  if (isAccountSqliteConfigured()) return toPublicAccount(sqliteGetCurrentAccount(tokenHash, nowIso, new Date(now.getTime() - SESSION_TOUCH_INTERVAL_MS).toISOString()) ?? {});
