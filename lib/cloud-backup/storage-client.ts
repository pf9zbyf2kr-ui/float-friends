...
function isAccountManaged(config: CloudBackupConfig): boolean {
  return config.mode === "account";
}

function managedObjectUrl(path: string): string {
  return `/api/account-cloud-storage?path=${encodeURIComponent(path.replace(/^\/+/, ""))}`;
}

async function managedControl(body: Record<string, unknown>, what: string): Promise<Response> {
  return fetchWithTimeout("/api/account-cloud-storage", {
    method: "POST",
    credentials: "include",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(body),
    cache: "no-store",
  }, CONTROL_TIMEOUT_MS, what);
}
...
export async function putObject(config: CloudBackupConfig, path: string, body: BlobPart, contentType = "application/octet-stream"): Promise<void> {
  ...
  await withRetries(async () => {
    ...
    if (!isAccountManaged(config) && !creds) throw new Error("未配置 Supabase 地址或 key。");
    const res = await fetchWithTimeout(isAccountManaged(config) ? managedObjectUrl(path) : objectUrl(creds!, path), {
      method: isAccountManaged(config) ? "PUT" : "POST",
      credentials: isAccountManaged(config) ? "include" : undefined,
      headers: isAccountManaged(config)
        ? { "Content-Type": contentType }
        : { ...authHeaders(creds!.key), "Content-Type": contentType, "x-upsert": "true" },
      body: blob,
    }, timeoutMs, "上传");
...
export async function getObject(config: CloudBackupConfig, path: string, onBytes?: DownloadProgress): Promise<Blob | null> {
  ...
  return withRetries(async () => {
    const res = await fetchWithTimeout(
      isAccountManaged(config) ? managedObjectUrl(path) : objectUrl(creds!, path),
      isAccountManaged(config)
...
export async function claimObject(config: CloudBackupConfig, path: string): Promise<boolean> {
  ...
  const res = await fetchWithTimeout(
    ...
    isAccountManaged(config)
      ...
      : { method: "DELETE", headers: authHeaders(creds!.key) },
    CONTROL_TIMEOUT_MS,
    "删除",
  );
...
export async function listObjects(config: CloudBackupConfig, prefix = "", limit = 100): Promise<StorageObject[]> {
  const creds = resolveCreds(config);
  if (!isAccountManaged(config) && !creds) throw new Error("未配置 Supabase 地址或 key。");
  const res = await withRetries(() => isAccountManaged(config)
    ? managedControl({ action: "list", prefix, limit }, "列举")
    : fetchWithTimeout(`${creds!.url}/storage/v1/object/list/${CLOUD_BACKUP_BUCKET}`, {
      method: "POST",
...
export async function ensureBucket(config: CloudBackupConfig): Promise<void> {
  if (isAccountManaged(config)) {
    const res = await managedControl({ action: "ensure" }, "创建存储桶");
    if (!res.ok) throw new Error(await describeError(res));
    return;
  }
  const creds = resolveCreds(config);
