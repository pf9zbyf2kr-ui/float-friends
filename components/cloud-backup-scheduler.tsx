...
import { createAccountCloudBackupConfig, isCloudBackupConfigured, loadCloudBackupConfig, saveCloudBackupConfig } from "@/lib/cloud-backup/config";
...
export function CloudBackupScheduler() {
  ...
  useEffect(() => {
    ...
    if (account.id !== "local_user" && (existing.mode !== "account" || existing.managedAccountId !== account.id)) {
      saveCloudBackupConfig(createAccountCloudBackupConfig(account.id));
    }
