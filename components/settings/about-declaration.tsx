...
import { Code2, ExternalLink, Heart, Info, Scale, ShieldAlert } from "lucide-react";

export function AboutDeclaration() {
    const sourceCodeUrl = (process.env.NEXT_PUBLIC_SOURCE_CODE_URL || "https://github.com/afufu/float-friends").replace(/\/$/, "");
    ...
    return (
        <div className="flex flex-col gap-5 h-full">
            ...
            <div className="flex flex-col gap-2">
                <a
                    className="g-card flex-row items-center"
                    href={sourceCodeUrl}
                    target="_blank"
                    rel="noreferrer"
                >
                    <Code2 size={20} className="shrink-0 text-[var(--c-icon-active)]" />
                    <span className="menu-label flex-1">获取对应版本源码</span>
                    <ExternalLink size={17} className="shrink-0 text-[var(--c-text-muted)]" />
                </a>
                <a
                    className="g-card flex-row items-center"
                    href={`${sourceCodeUrl}/blob/main/LICENSE`}
                    target="_blank"
                    rel="noreferrer"
                >
                    <Scale size={20} className="shrink-0 text-[var(--c-icon-active)]" />
                    <span className="menu-label flex-1">AGPL-3.0 许可证</span>
                    <ExternalLink size={17} className="shrink-0 text-[var(--c-text-muted)]" />
                </a>
