...
import { formatSupabaseRestError } from "@/lib/server/supabase-rest";
...
export async function POST(request: Request) {
  try {
    if (!isAccountStorageConfigured()) {
      return NextResponse.json({ ok: false, error: "账号存储尚未配置。" }, { status: 503 });
    }
    ...
    if (user) {
      ...
      if (!activationCode && !openRegistration) {
      ...
      if (openRegistration && !activationCode) {
        try {
          user = await createUser({ username, password, displayName });
        } catch (err) {
          const message = err instanceof Error ? err.message : String(err);
          if (/username_taken|unique|duplicate/i.test(message)) {
            return NextResponse.json({ ok: false, error: "该账号已被注册。" }, { status: 409 });
          }
          throw err;
        }
      } else {
    ...
    return NextResponse.json(
      ...
      { status: isAccountStorageConfigured() ? 400 : 503 },
    );
  }
}
